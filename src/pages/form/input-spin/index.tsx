...
interface SkinSpinInputProps {
  initialCount: number;
}

const SkinSpinRightInput: React.FC<SkinSpinInputProps> = ({ initialCount }) => {
  const [count, setCount] = useState(initialCount);
  const handleDecrease = () => setCount(count - 1);
  const handleIncrease = () => setCount(count + 1);
...
const SkinSpinLiftInput: React.FC<SkinSpinInputProps> = ({ initialCount }) => {
  const [count, setCount] = useState(initialCount);
  const handleDecrease = () => setCount(count - 1);
  const handleIncrease = () => setCount(count + 1);
