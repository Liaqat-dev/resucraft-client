...
const ProjectsLists: NextPageWithLayout = () => {
  ...
  const handleCheckboxChange = (assignee: AssigneeType) => {
    setSelectedAssignees((prevSelectedAssignees) => {
      const isSelected = prevSelectedAssignees.some(
        (selected) => selected._id === assignee._id,
      );
      if (isSelected) {
  ...
  const onClickProjectListDelete = (_id: number) => {
    setProjectLists([_id]);
    setShow(true);
  };
  ...
  const handleOpenModal = useCallback(
    (editMode: boolean = false, projectList: ProjectList | null = null) => {
      setEditMode(editMode);
      setCurrentProjectList(projectList);
          ...
          return (
            <>
              <div className="flex ml-3 -space-x-3 grow">
                {value.getValue().map((item: any, idx: number) => (
                  <Link
                    to="#!"
          ...
          <div className="grid grid-cols-12 gap-5">
            <div className="col-span-12 xl:col-span-4">
              <div className="relative group/form">
                ...
                />
                <div className="absolute inset-y-0 flex items-center text-gray-500 dark:text-dark-500 ltr:left-3 rtl:right-3 group-[&.right]/form:right-3 group-[&.right]/form:left-auto focus:outline-none">
              ...
              <div>
                <Dropdown position="right" dropdownClassName="dropdown">
                  ...
                  <DropdownMenu menuclass="!fixed p-2 !w-52">
                    ...
                    {assignee.map((item) => (
                      <div className="py-2 input-check-group" key={item._id}>
                        <input
                          id={item._id}
