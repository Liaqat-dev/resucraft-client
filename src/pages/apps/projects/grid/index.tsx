...
const ProjectsGrid: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    document.title =
      "Projects Grid | Domiex - React TS Admin & Dashboard Template";
  }, []);
  ...
  const onClickProjectGridDelete = (_id: number) => {
    setProjectGrids([_id]);
    setShow(true);
  };
  ...
  const paginateProjects = (projects: any[]) => {
          ...
          <Tab label="All Projects">
            <div className="grid grid-cols-1 mt-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-5">
              {paginatedProjects &&
                ...
                paginatedProjects.map((project: any, idx: any) => (
                  <ProjectCard
                    key={idx}
                    project={project}
