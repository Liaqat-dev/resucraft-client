...
const List: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    document.title =
      "Products List | Domiex - React TS Admin & Dashboard Template";
  }, []);
  ...
  const handleSelectAll = useCallback(() => {
    if (selectAll) {
      ...
      setDeletedListData(
        allProductList.map((order: { _id: any }) => order._id),
      );
    }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        header: "Price",
        accessorKey: "price",
        cell: (value: any) => <span>${value.getValue()}</span>,
      },
      {
        header: "QTY",
        accessorKey: "qty",
  ...
  return (
    <React.Fragment>
      <BreadCrumb subTitle="E-commerce" title="Product List" />
      {/* header */}
      <div className="card">
        {/* card header  */}
        <div className="card-header">
          <div className="flex flex-wrap items-center gap-5">
            <div className="grow">
              <h6 className="mb-1 card-title">Product List</h6>
