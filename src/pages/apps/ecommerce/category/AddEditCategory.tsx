  ...
  useEffect(() => {
    ...
    if (editMode && currentCategoryList) {
      ...
      Object.keys(currentCategoryList).forEach((key) => {
        setValue(key as keyof CategoryItems, (currentCategoryList as any)[key]);
      });
  ...
  const submitForm = (data: CategoryItems) => {
    if (editMode && currentCategoryList) {
      ...
      const newCategoryList = {
        ...
        image: preview || "",
        categoryID: getRecordId(categoryData, "categoryID", "PEC"),
      };
              ...
              <div className="grid grid-cols-12 gap-5">
                <div className="col-span-12">
                  <div>
                    <label
                      htmlFor="logo"
                      className="flex items-center justify-center p-2 mx-auto overflow-hidden bg-gray-100 border border-gray-200 rounded-smcursor-pointer dark:bg-dark-850 dark:border-dark-800 size-32"
                    >
                      {preview ? (
                        <img
                          src={preview}
