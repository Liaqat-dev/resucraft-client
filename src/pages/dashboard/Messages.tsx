...
import { contactService, ContactMessage, ContactStats } from '@src/services/contact.service';
...
const STAT_ITEMS = [
    ...
    { key: 'replied', label: 'Replied', icon: <Reply className="size-4" />, color: 'text-emerald-600', bg: 'bg-emerald-500/10' },
    { key: 'archived', label: 'Archived', icon: <Archive className="size-4" />, color: 'text-violet-500', bg: 'bg-violet-500/10' },
];
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <div
                ...
                style={{ background: 'linear-gradient(135deg, #1a1f2e 0%, #141824 100%)', border: '1px solid rgba(255,255,255,0.08)' }}
            >
                {/* Header */}
                ...
                <div className="px-5 py-4">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-slate-600 block mb-2">Your Reply</label>
                    <textarea
                        value={text}
...
function DetailPanel({ message, onClose, onReply, onArchive, onDelete }: {
    ...
    return (
        <div className="flex flex-col h-full" style={{ animation: 'admFadeIn 0.18s ease' }}>
            {/* Header */}
            <div className="flex items-center gap-2 px-4 py-3.5 shrink-0 border-b border-gray-100 dark:border-white/[0.06]">
                <button
                    onClick={onClose}
                ...
                <span className="adm-display text-sm font-semibold text-gray-800 dark:text-slate-200 flex-1 min-w-0 truncate">
                    {message.topic}
                </span>
            ...
            {/* Sender info */}
            <div className="flex items-center gap-3 px-4 py-3.5 shrink-0 border-b border-gray-100 dark:border-white/[0.04]">
                <div className="size-10 rounded-xl flex items-center justify-center text-base font-bold shrink-0 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20">
            ...
            {/* Actions */}
            <div className="flex items-center gap-2 px-4 py-3.5 shrink-0 border-t border-gray-100 dark:border-white/[0.06]">
                <button
                    onClick={onReply}
...
export default function Messages() {
    ...
    const handleArchive = async (msg: ContactMessage) => {
        ...
        } catch (e: any) { showToast(e?.response?.data?.message || 'Archive failed', 'err'); }
    };
    ...
    const handleDelete = async (msg: ContactMessage) => {
        try {
            ...
            setTotal(t => t - 1);
        } catch (e: any) { showToast(e?.response?.data?.message || 'Delete failed', 'err'); }
    };
    ...
    const handleReply = async (text: string) => {
        ...
        } catch (e: any) { showToast(e?.response?.data?.message || 'Failed to send reply', 'err'); }
        finally { setSending(false); setReplyTarget(null); }
    };
        ...
        <>
            ...
            <div className="adm-page min-h-full text-gray-800 dark:text-slate-300">
                ...
                {toast && (
                    ...
                    >
                        {toast.type === 'ok' ? <CheckCircle2 className="size-4 shrink-0" /> : <XCircle className="size-4 shrink-0" />}
                        {toast.msg}
                    </div>
                )}
                ...
                {/* ── Reply modal ── */}
                {replyTarget && (
                    <ReplyModal message={replyTarget} onSend={handleReply} onCancel={() => setReplyTarget(null)} sending={sending} />
                )}
                ...
                <div className="mb-6 flex items-end gap-4 flex-wrap">
                    <div>
                        ...
                        <h1 className="adm-display text-2xl font-bold text-gray-900 dark:text-slate-100" style={{ letterSpacing: '-0.02em' }}>
                            Customer Support
                        </h1>
                    </div>
                    <div className="ml-auto flex items-center gap-2">
                        {stats && stats.unread > 0 && (
                            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-500/10 border border-amber-500/25 text-amber-600 dark:text-amber-400">
                                <span className="size-1.5 rounded-full bg-amber-500 inline-block" style={{ animation: 'admFadeIn 2s ease-in-out infinite alternate' }} />
                                {stats.unread} unread
                            </div>
                        )}
                        {stats && (
                            <span className="adm-mono text-xs px-2.5 py-1 rounded-full bg-black/5 dark:bg-white/5 border border-black/8 dark:border-white/8 text-gray-500 dark:text-slate-500">
                ...
                <StatCardGrid
                    ...
                    values={statValues}
                    loading={loading}
                    onCardClick={key => { setStatusFilter(key === 'total' ? '' : key); setPage(1); setSelected(null); }}
                />

                {/* ── Error ── */}
                {error && (
                    <div className="flex items-center gap-3 px-4 py-3 rounded-xl mb-4 text-sm bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400">
                        <XCircle className="size-4 shrink-0" />
                        {error}
                        <button onClick={() => fetchData()} className="ml-auto text-xs underline underline-offset-2 opacity-70 hover:opacity-100">Retry</button>
                    </div>
                )}
                ...
                >
                    {/* ── Left: list ── */}
                    <div className="flex flex-col" style={{ borderRight: selected ? '1px solid rgba(0,0,0,0.07)' : 'none' }}>

                        {/* Filters */}
                        <div className="adm-filter-bar flex flex-wrap gap-2 p-3">
                            <div className="relative flex-1 min-w-[160px]">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-3.5 text-gray-400 dark:text-slate-600 pointer-events-none" />
                                <input
                                    type="text"
                                ...
                                />
                                {search && (
                                    <button onClick={() => setSearch('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:text-slate-600 dark:hover:text-slate-400">
                                        <X className="size-3.5" />
                                    </button>
                                )}
                            </div>
                            <div className="relative">
                                <Filter className="absolute left-2.5 top-1/2 -translate-y-1/2 size-3.5 pointer-events-none text-gray-400 dark:text-slate-600" />
                                <ChevronsUpDown className="absolute right-2 top-1/2 -translate-y-1/2 size-3 pointer-events-none text-gray-400 dark:text-slate-600" />
                                <select
                            ...
                            {(statusFilter || search) && (
                                <button
                                    onClick={() => { setStatusFilter(''); setSearch(''); setPage(1); setSelected(null); }}
                        ...
                        {/* Count bar */}
                        <div className="flex items-center justify-between px-4 py-2 border-b border-black/[0.05] dark:border-white/[0.04]">
                            ...
                            {statusFilter && STATUS_META[statusFilter as ContactMessage['status']] && (
                                <span className={`adm-mono text-[10px] font-bold px-2 py-0.5 rounded-full border ${STATUS_META[statusFilter as ContactMessage['status']].cls}`}>
                                            ...
                                            >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-baseline justify-between gap-2 mb-0.5">
                                                        <span className={`text-sm truncate ${isUnread ? 'font-semibold text-gray-900 dark:text-slate-100' : 'font-medium text-gray-600 dark:text-slate-400'}`}>
                                                            {msg.name}
                                                        </span>
                                                        <span className="adm-mono text-[10px] text-gray-400 dark:text-slate-600 shrink-0">
                                                            {timeAgo(msg.createdAt)}
                                                        </span>
                                                    </div>
                                                    <span className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mb-1.5 ${topicMeta.cls}`}>
                                                        {msg.topic}
                                                    </span>
                                                ...
                                                <span className={`adm-mono text-[9px] font-bold px-1.5 py-0.5 rounded-full border shrink-0 ${meta.cls}`}>
                                                    {meta.label}
                                                </span>
                                            </div>
                                        );
                        ...
                        {pages > 1 && (
                            <div className="flex items-center justify-between px-4 py-3 border-t adm-divider">
                                <span className="adm-mono text-xs text-gray-400 dark:text-slate-600">Page {page} / {pages}</span>
                                <div className="flex items-center gap-1">
                                    <button className="adm-page-btn" disabled={page <= 1} onClick={() => { setPage(p => p - 1); setSelected(null); }}>
                                        <ChevronLeft className="size-4" />
                                    </button>
                                    {Array.from({ length: Math.min(pages, 5) }, (_, i) => {
                                        const p = i + 1;
                                        return (
                                            <button key={p} className={`adm-page-btn ${page === p ? 'active' : ''}`} onClick={() => { setPage(p); setSelected(null); }}>
                                                {p}
                                            </button>
                                        );
                                    })}
                                    <button className="adm-page-btn" disabled={page >= pages} onClick={() => { setPage(p => p + 1); setSelected(null); }}>
                ...
                {!loading && !error && messages.length === 0 && !statusFilter && !search && (
                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                        <div className="size-16 rounded-2xl flex items-center justify-center bg-indigo-500/10 border border-indigo-500/20">
                            <MailOpen className="size-7 text-indigo-500" />
                        </div>
                        <p className="font-semibold text-gray-500 dark:text-slate-500">No messages yet</p>
                        <p className="text-sm text-gray-400 dark:text-slate-600">Contact messages from users will appear here.</p>
                    </div>
                )}
            </div>
        </>
