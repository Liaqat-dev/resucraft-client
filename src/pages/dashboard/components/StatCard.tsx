...
export function StatCard({ item, value, delay = 0, onClick }: StatCardProps) {
    return (
        <div
            className={`adm-card adm-stat-card p-4 ${onClick ? 'cursor-pointer' : ''}`}
            style={{ animation: `admStatIn 0.2s ease ${delay}ms both` }}
            onClick={onClick}
        >
            <div className={`inline-flex items-center justify-center size-8 rounded-lg mb-2.5 ${item.bg}`}>
                <span className={item.color}>{item.icon}</span>
            </div>
            <p className="adm-mono text-xl font-bold text-gray-800 dark:text-slate-200 adm-counter">
