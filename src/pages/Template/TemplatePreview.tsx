...
import {AlertTriangle, ArrowLeft, Loader2, Mic, Sparkles, UserCheck} from 'lucide-react';
...
import TemplateReviews from "@pages/Template/TemplateReviews.tsx";
...
const TemplatePreview = () => {
    ...
    return (<div className="min-h-screen px-3 py-4">
            <div className="max-w-7xl mx-auto">
                ...
                {loadingTemplate ? (
                    <div className="flex flex-col items-center justify-center min-h-96 gap-3">
                        <div
                            className="size-10 rounded-full border-4 border-gray-200 border-t-primary-500 animate-spin"/>
                    ...
                    <div className="max-w-sm mx-auto mt-16 bg-red-50 border border-red-200 rounded-xl p-6 text-center">
                        ...
                        <div className="flex-1 min-w-0 max-w-[500px]">
                            <div className=" rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 mt-1">
                                        Paste the job description and we'll fill this template with content tailored to
                                        the
                                        role.
                                    </p>
                                </div>
                                ...
                                <div
                                    className={`rounded-lg p-4 ${profileReady ? 'bg-green-50 border border-green-200' : 'bg-amber-50 border border-amber-200'}`}>
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2">
                                            <UserCheck
                                                className={`size-4 ${profileReady ? 'text-green-600' : 'text-amber-600'}`}/>
                                            <span
                                                className={`text-sm font-semibold ${profileReady ? 'text-green-700' : 'text-amber-700'}`}>
                                                Profile Completion
                                            </span>
                                        </div>
                                        <span
                                            className={`text-sm font-bold ${profileReady ? 'text-green-700' : 'text-amber-700'}`}>
                                            {percentage}%
                                        </span>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows={8}
                                        className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm  placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none transition"
                                    />
                        ...
                        <div className="w-full mt-8">
                            {id && <TemplateReviews templateId={id}/>}
                        </div>
                    </div>
                )}
            </div>
