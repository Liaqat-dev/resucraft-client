...
const AddEditStaffList = ({
  ...
  useEffect(() => {
    ...
    if (editMode && currentStaff) {
      Object.keys(currentStaff).forEach((key) => {
        setValue(key as keyof StaffList, (currentStaff as any)[key]);
      });
  ...
  const generateNewStaffId = (staffList: any) => {
    // Extract the numeric part of all existing staff IDs and find the maximum
    const maxStaffId =
      staffList.length > 0
        ? Math.max(
            ...staffList.map((staff: any) => {
              const numericPart = parseInt(staff.staffId.split("-")[1], 10);
    ...
    const newStaffId = maxStaffId + 1;
    return `PES-${newStaffId}`;
  };
