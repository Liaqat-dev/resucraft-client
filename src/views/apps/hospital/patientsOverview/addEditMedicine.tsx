  ...
  useEffect(() => {
    if (editMode && currentContact) {
      Object.keys(currentContact).forEach((key) => {
        setValue(key as keyof Medicine, (currentContact as any)[key]);
      });
      setSelectedStartDate(new Date(currentContact.startDate));
      setSelectedEndDate(new Date(currentContact.endDate));
    } else {
      reset({
        _id: 0,
        date: "",
