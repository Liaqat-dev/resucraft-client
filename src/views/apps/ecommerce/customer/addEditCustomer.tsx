...
const subscribeOptions: OptionType[] = [
  { label: "Yes", value: "Yes" },
  { label: "No", value: "No" },
];
...
const AddEditNewCustomer: React.FC<AddNewCustomerPropsModal> = ({
  ...
  const getRecordId = (list: any, key: any, prefix: any) => {
    // Find the highest ID in the list
    const lastId = list
      .map((item: any) => item[key])
      .filter((id: any) => id.startsWith(prefix))
      .map((id: any) => parseInt(id.replace(prefix + "-", ""), 10))
  ...
  const submitForm = (data: CustomerRecord, onClose: () => void) => {
    if (editMode && currentCustomer) {
      ...
      const newCustomer = {
        ...data,
        _id: customerList.length > 0 ? customerList.length + 1 : +1,
  ...
  useEffect(() => {
    if (editMode && currentCustomer) {
      ...
      Object.keys(currentCustomer).forEach((key) => {
        setValue(key as keyof CustomerRecord, (currentCustomer as any)[key]);
      });
