        ...
        <div className="text-center">
          <div className="relative inline-block mx-auto">
            <div className="relative p-1 rounded-full bg-gradient-to-tr from-primary-300 via-red-300 to-green-300 -mt-14">
              <img
                  src={user?.avatarUrl || getAvatar(user.name)}
                  alt="userImg"
                  className="rounded-full size-11 bg-cover"
              />
            </div>
            <div className="absolute border-2 border-white dark:border-dark-900 rounded-full size-4 bg-green-500 bottom-1 ltr:right-1 rtl:left-2.5"></div>
          </div>
          <h5 className="mt-2 mb-1 ">
            {user?.username}
            {user?.isVerified?<BadgeCheck className="inline-block text-primary-500 fill-primary-500/20 size-5"></BadgeCheck>:null}
          </h5>
